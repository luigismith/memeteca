...
# 27 agosto 2026. Una sessione ha scritto cinque schede nuove — LERCIO,
# MI SI NOTA DI PIU', ITALIAN BRAINROT, SPINOZA, IL GABIBBO — che erano gia'
# in archivio come 022, 025, 026, 023 e 027, tutte gia' pubblicate. Il lunedi'
# successivo sarebbe uscito un doppione, con lo stesso titolo e le stesse
# tavole, sotto un numero diverso.
#
# L'errore e' nato leggendo scaletta.CANDIDATI a occhio: quella lista e'
# statica e tiene dentro anche i nomi gia' lavorati, perche' il filtro vive in
# prossimo_candidato(), che quella sessione non ha chiamato. Nessuno ha
# protestato: le tavole si sono generate, la caption stava nei limiti, i test
# passavano. Un doppione e' formalmente corretto in ogni suo campo.
#
# Da qui in poi protesta l'import. Costa un millisecondo e vale una figuraccia.
def _controlla_doppioni(memi):
    from collections import Counter
    for campo in ("num", "titolo"):
        doppi = {v: n for v, n in Counter(m[campo] for m in memi).items() if n > 1}
        if doppi:
            raise AssertionError(
                f"contenuti.py: {campo} duplicato in MEMI → {doppi}. "
                "Una scheda gia' in archivio non si riscrive: si controlla "
                "con scaletta.prossimo_candidato(), non leggendo CANDIDATI.")


_controlla_doppioni(MEMI)
